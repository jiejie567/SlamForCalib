...
void Atlas::AddNewMap(LoopClosing* mpLoopCloser,System* pSLAM2)
{
    // 锁住地图集
    unique_lock<mutex> lock(mMutexAtlas);
    cout << "Creation of new map with id: " << Map::nNextId << endl;
    ...
    cout << "Creation of new map with last KF id: " << mnLastInitKFidMap << endl;
    //Todo 把pSLAM2的当前地图的关键帧每一帧都加上mnLastInitKFidMap，设置为mpCurrentMap,设置为活跃地图，插入地图集
    Map* pLastMap = mpCurrentMap;
    mpCurrentMap = pSLAM2->GetAtlas()->GetCurrentMap();  //得到SLAM2的地图
    mpCurrentMap->SetCurrentMap();              //设置为活跃地图
    //TODO:把mpCurrentMap的所有keyframe的id加上mnLastInitKFidMap 维护新地图的其他变量
    std::vector<KeyFrame*> tmp = mpCurrentMap->GetAllKeyFrames();
    for(auto pKF:tmp)
    {
        mpLoopCloser->InsertKeyFrame(pKF);
        pKF->mnId = pKF->mnId+mnLastInitKFidMap;
    }
//    mpCurrentMap->nNextId = mpCurrentMap->nNextId + pLastMap->nNextId; //因为nNextId为静态变量，先不管，注释。
    mpCurrentMap->ChangeId(mpCurrentMap->GetId()+pLastMap->GetId());
    mspMaps.insert(mpCurrentMap);               //插入地图集
}
